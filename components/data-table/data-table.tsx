...
interface DataTableProps<TData extends ExportableData, TValue> {
  ...
  fetchDataFn: ((params: DataFetchParams) => Promise<DataFetchResult<TData>>) |
               ((page: number, pageSize: number, search: string, dateRange: { from_date: string; to_date: string }, sortBy: string, sortOrder: string, caseConfig?: CaseFormatConfig, customFilters?: any) => unknown);

  // Function to fetch specific items by their IDs
  fetchByIdsFn?: (ids: number[] | string[]) => Promise<TData[]>;

  // Custom filters for toolbar
  customFilters?: any;

  // Custom filter component to render in toolbar
  renderCustomFilters?: (filters: any, setFilters: (filters: any) => void) => React.ReactNode;
...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  const [customFilters, setCustomFilters] = useConditionalUrlState<any>("customFilters", initialCustomFilters || {});
  ...
  }, [page, pageSize, search, dateRange, sortBy, sortOrder, customFilters, fetchDataFn]);
  ...
  const queryResult = (fetchDataFn as { isQueryHook?: boolean }).isQueryHook === true
    ? (fetchDataFn as (page: number, pageSize: number, search: string, dateRange: { from_date: string; to_date: string }, sortBy: string, sortOrder: string, caseConfig?: CaseFormatConfig, customFilters?: any) => {
        isLoading: boolean;
        isSuccess: boolean;
        isError: boolean;
